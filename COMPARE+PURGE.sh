...
diffImg()
{
	...
	then
		...
		then
			...
			echo -e "\t [Image] IDENTICAL results reported by 'b2sum' ..."
			rm -f ${TMP}.diff
		else
			if [ -n "${IDIFF}" ]
			then
...
diffBin()
{
	...
	then
		...
		then
			...
			if [ "${sumK}" = "${sumL}" ]
			then
				if [ ${VERBl} -eq 1 ] ; then  echo -e "\t      [6C-1a2] ..." >&2 ; fi
	...
	then 
		...
		then
			...
			then
				...
				testBin=$(file "${fileA}" | cut -f2 -d: | grep 'gzip compressed data' ) ;
				if [ -z "${testBin}" ]
				then
					testBin=$(file "${fileA}" | cut -f2 -d: | grep 'Zip archive data' ) ;
				fi

				echo -e "\t\t\t ### testBin[1] = '${testBin}' ..." ;
				if [ -n "${testBin}" ]
				then
       					isBin=2 ;
					if [ ${VERBl} -eq 1 ] ; then  echo -e "\t     [5Da] ..." >&2 ; fi ;

				else
					testBin=$(file "${fileA}" | cut -f2 -d: | grep 'executable' ) ;

					echo -e "\t\t\t ### testBin[2] = '${testBin}' ..." ;
					if [ -n "${testBin}" ]
					then
       						isBin=1 ;
						if [ ${VERBl} -eq 1 ] ; then  echo -e "\t     [5Db1] ..." >&2 ; fi ;
					else
						isText=1 ;
       						isBin=0 ;
						if [ ${VERBl} -eq 1 ] ; then  echo -e "\t     [5Db2] ..." >&2 ; fi ;
					fi
				fi
...
if [ -s ${TMP}.remainder -a ${doSkip} -eq 0 ]
then
